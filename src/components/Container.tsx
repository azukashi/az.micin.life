...
const Container: React.FC<ContainerProps> = ({ asChild, className, children, variant, ...props }) => {
    const Comp = asChild ? React.Fragment : 'div';
    const containerClasses = cn(containerVariants({ variant }), className);

    return (
        <Comp className={containerClasses} {...props}>
            {variant === 'narrowConstrainedPadded' ? <div className="mx-auto max-w-3xl">{children}</div> : children}
        </Comp>
    );
};
